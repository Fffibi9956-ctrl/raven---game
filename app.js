...
function createFlightPath() {
    ...
    // Точки, через которые пролетит ворон
    const curvePoints = [
        new THREE.Vector3(-20, 5, -5),   // Начало: слева, сверху, сзади
        new THREE.Vector3(-10, 4, -2),   // Приближение к центру
        new THREE.Vector3(0, 3, 0),      // Центр (здесь сбросит фрагмент)
        new THREE.Vector3(10, 5, 3),     // Удаление вправо
        new THREE.Vector3(20, 6, 5)      // Конец: справа, сверху, впереди
    ];
...
function loadRavenModel() {
    ...
    // 🔴 ВАЖНО: Используем jsDelivr для обхода CORS-ограничений GitHub
    // Формат: https://cdn.jsdelivr.net/gh/ВАШ_ЛОГИН/ВАШ_РЕПОЗИТОРИЙ/ФАЙЛ
    ...
    loader.load(
        ...
        function(gltf) {
            ...
            // Настройка размера и положения модели
            ravenModel.scale.set(0.8, 0.8, 0.8); // Масштабируем
        ...
        function(error) {
            ...
            // СОЗДАЁМ ТЕСТОВЫЙ ОБЪЕКТ вместо модели
...
function createMapFragment() {
    ...
    console.log('Фрагмент карты создан');
}
...
function animate() {
    ...
    if (ravenModel && progress < 1) {
        ...
        // 3. ЛЁГКОЕ ПОКАЧИВАНИЕ ДЛЯ ЭФФЕКТА ПАРЕНИЯ
